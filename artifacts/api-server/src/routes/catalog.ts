...
router.get("/catalog/products/:id", async (req, res): Promise<void> => {
  ...
  const [row] = await db.select({
    ...
    imageUrl: sql<string | null>`coalesce(${productOverridesTable.imageUrl}, ${productsTable.imageUrl})`,
     salePrice: sql<string | null>`coalesce(min(${stockBatchesTable.salePrice}), min(${stockBatchesTable.mrp}))`,
    ...
    .leftJoin(drugsTable, eq(productsTable.drugId, drugsTable.id))
    ...
    .groupBy(productsTable.id, companiesTable.name, drugsTable.drugName, categoriesTable.name, categoriesTable.displayName, productOverridesTable.imageUrl, productOverridesTable.prescriptionRequired);
  if (!row || !row.active) {
...
router.post("/catalog/products/:id/image-signature", async (req, res): Promise<void> => {
  ...
  const folder = "ayush-foundation/products";
