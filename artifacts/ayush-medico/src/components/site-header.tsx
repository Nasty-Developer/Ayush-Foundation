import { ArrowRight, Menu, Phone, ShoppingBag, X } from 'lucide-react';
      ...
      <div className="site-container flex h-[76px] items-center justify-between">
        <Link to="/" className="flex items-center gap-3" onClick={() => setIsOpen(false)} data-testid="link-brand-home">
          ...
          <span className="leading-none">
            <span className="block font-display text-[1.32rem] font-semibold tracking-[-0.03em] text-foreground">Ayush Medico</span>
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <div className="relative">
            <button type="button" className="flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition-colors hover:border-primary/30 hover:bg-secondary hover:text-primary" onClick={() => setCartNotice((notice) => !notice)} aria-label="Open bag placeholder" aria-expanded={cartNotice} data-testid="button-header-cart">
              <ShoppingBag size={18} />
            </button>
            {cartNotice && <p className="absolute right-0 top-12 z-10 w-44 rounded-xl border border-border bg-card p-3 text-xs font-semibold leading-5 text-muted-foreground shadow-md">The bag is a visual placeholder for a future service.</p>}
          </div>
