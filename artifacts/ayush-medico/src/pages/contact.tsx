import { ArrowRight, Clock3, MapPin, MessageCircle, Navigation, Phone, Send } from 'lucide-react';
...
export default function ContactPage() {
  return (
    <PageFrame eyebrow="Contact us" title="We’re just a call away." description="For medicine availability, delivery questions, or anything else on your mind, reach out to the Ayush Medico team.">
      <section className="site-container grid gap-8 py-14 md:grid-cols-[0.85fr_1.15fr] md:py-20">
        ...
        <div className="rounded-[1.75rem] border border-border bg-card p-7 shadow-sm sm:p-9">
          ...
          <p className="mt-3 max-w-lg text-sm leading-6 text-muted-foreground">For medicine availability, delivery questions, or a quick location check, call or message the Ayush Medico team during our opening hours.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <a href={whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-border px-5 py-3 text-sm font-bold text-primary transition-colors hover:bg-secondary" data-testid="link-contact-whatsapp">WhatsApp Us <MessageCircle size={16} /></a>
            <a href={directionsUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-border px-5 py-3 text-sm font-bold text-primary transition-colors hover:bg-secondary" data-testid="link-contact-directions">Get Directions <Navigation size={16} /></a>
          </div>
